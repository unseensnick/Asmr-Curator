...
Comments, docstrings, `README.md` lines, or `Handoff.md` claims that contradict current code. One line each: what it says, and what is actually true. Surface for pruning, do not silently fix.

### 4. The plan

Steps as **bolded named items** (`Step 1a, validate the root before resolving the path`), each two or three sentences covering what it does and why it is safe at that point in the sequence. Ordering is the content: say what a step depends on and what it unblocks.
