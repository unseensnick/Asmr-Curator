...
- **`backend/private/`** — optional private-only domain (routes, helpers and their deps). `main.py` registers its router inside a `try/ModuleNotFoundError` guard, so the directory can be absent. Nothing outside it may import from it. The frontend and extension mirror this with `frontend/src/private/` and `extension/private/`.
