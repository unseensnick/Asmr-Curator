...
argument-hint: "<research question> (e.g. 'where are our test gaps', 'how does an ingest reach disk end to end', 'audit the routes for path-traversal risk')"
...
- Broad or deep questions spanning many files or whole subsystems: "where are our test gaps", "how does a Patreon fetch reach the library end to end", "audit every route that shells out", "where is untrusted input handled", "what's dead code".
...
- Pre-planning ONE concrete task (add this route, rework this panel). Use `/scout`, it is lighter and task-scoped.
...
- **Adversarial verification is mandatory**, not a scale knob. Re-read the claims that would most distort the plan if wrong. That explicitly includes claims from `Handoff.md`, memories, and subagent findings, all of which have been wrong in this repo before.
...
Echo the research question in one sentence so the user can correct a misread before agents spend tokens. If it's vague (which subsystem? what aspect? what counts as a "gap" or "risk"?), use `AskUserQuestion` to narrow, do not fan out on a fuzzy question. Decide the **angle of decomposition** (by module, by request flow, by concern) and the **inclusion bar** (what counts as a finding vs noise).
...
- The relevant files and modules (`Glob`, `Grep`), and for test questions, the existing inventory under `backend/tests/` and `frontend/src/**/*.test.ts*` so agents don't re-flag covered ground.
- `git log --oneline -20` for recent shape; `Handoff.md`, `.claude/rules/*`, and `CHANGELOG.md` `[Unreleased]` for known constraints and deliberate descopes (the most common audit failure is flagging deferred work as missing).
- Note what is already covered or out of scope, verbatim, to hand to every agent.

### 3. Fan out parallel explorers (the "searches")

Split the question into 3-6 independent areas or angles and spawn one `Agent` (`subagent_type: Explore`) per area, **all in one message so they run in parallel**. Natural seams in this repo: backend routes, backend helpers, the Playwright and subprocess layer, `frontend/src/lib/`, `frontend/src/components/`, the extension.

Brief each as a smart colleague who hasn't seen this conversation:

- State the goal and the inclusion bar (what's a finding, what's noise).
- Hand over exact paths and globs and the **already-covered / out-of-scope list** so they don't re-flag it.
...
- **Tell them the interpreter.** The backend requires Python >= 3.14 (`backend/pyproject.toml`). An agent testing syntax under an older CPython has already reported valid 3.14 code as broken.
...
- "X is untested" when a test exists; "X doesn't exist" when it moved or was renamed.
...
- A syntax or API claim made against the wrong Python version, or against a library version this repo doesn't pin.
- Misjudged testability (for example "add a unit test" for logic that shells out to ffmpeg or drives Playwright and would need a refactor first).

Trust current code over memory, `Handoff.md`, and agent text, all of which have asserted things in this repo that current code contradicted. A claim carried forward from a handoff is a hypothesis exactly like a subagent's, and gets the same re-read when it is load-bearing.
...
- **Note testability honestly** when the question is about tests: pure and easy, needs a boundary mock (network, filesystem, clock), or needs a refactor first. Per [.claude/rules/testing.md](../../rules/testing.md), prefer real implementations and mock only at system boundaries.
- **Report the plan section as a plan**, not as "recommended next step" prose, whenever the research is feeding implementation.

### 6. Hand off

End with "Ready to act" plus the recommended first batch, or "Open questions block this". Do not start writing tests, fixes, or refactors; the user decides whether and what to action.
...
- Every concrete claim cites `file:line` from current code. Memory, `Handoff.md`, and agent-summary claims are hypotheses until cited.
