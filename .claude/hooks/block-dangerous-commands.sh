...
# git checkout -- <path>  /  git checkout HEAD -- <path>  /  git restore <path>
# All three discard uncommitted working-tree changes.
if contains_cmd 'git[[:space:]]+checkout([[:space:]]+[^[:space:]]+)*[[:space:]]+--[[:space:]]'; then
  emit_deny "Blocked: git checkout -- <file> discards uncommitted changes. Stash or commit first."
fi
if contains_cmd 'git[[:space:]]+restore([[:space:]]+--[a-zA-Z-]+)*[[:space:]]+[^[:space:]]'; then
  emit_deny "Blocked: git restore discards uncommitted changes. Stash or commit first."
fi
# git stash drop / pop / clear can destroy stashed work without a recovery path.
if contains_cmd 'git[[:space:]]+stash[[:space:]]+(drop|clear|pop)'; then
  emit_deny "Blocked: git stash drop/clear/pop can destroy stashed work. Apply, inspect, then drop manually."
fi
# git branch -D / --delete --force
if contains_cmd 'git[[:space:]]+branch[[:space:]]+(-D|--delete[[:space:]]+--force|--force[[:space:]]+--delete)'; then
  emit_deny "Blocked: git branch -D force-deletes a branch even if unmerged. Use -d (lowercase) after confirming the merge."
fi
