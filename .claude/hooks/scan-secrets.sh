...
# Patreon session cookie (the only credential this app handles). Looks like a
# long hex/alnum string assigned to a session_id / patreon_cookie field.
if echo "$CONTENT" | grep -qiE '(session_id|patreon[_-]?(cookie|session))[[:space:]]*[=:][[:space:]]*["\x27]?[A-Za-z0-9]{40,}' && \
   ! echo "$CONTENT" | grep -qiE '(session_id|patreon[_-]?(cookie|session))[[:space:]]*[=:][[:space:]]*["\x27]?(process\.env|os\.environ|getenv|\$\{|ENV\[|env\(|<|\.\.\.|placeholder|example|XXXX|YOUR_)'; then
  MATCHES="$MATCHES Patreon session cookie;"
fi
