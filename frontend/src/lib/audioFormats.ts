...
export const FORMAT_VALUES = audioFormatsConfig.outputFormats.map((f) => f.value) as ConvertFormat[];
export const QUALITY_VALUES = audioFormatsConfig.qualityLevels.map((q) => q.value) as ConvertQuality[];
