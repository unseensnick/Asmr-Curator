...
export default function CookiePane({ open }: CookiePaneProps) {
    ...
    useEffect(() => {
        if (!open) return;
        let cancelled = false;
        Promise.allSettled([getPatreonCookieStatus(), getGoogleCookieStatus()]).then(([p, g]) => {
            if (cancelled) return;
            setPatreonStatus(p.status === "fulfilled" ? p.value : { set: false, length: 0 });
            setGoogleStatus(
                g.status === "fulfilled" ? g.value : { set: false, count: 0, length: 0 },
            );
        });
        return () => {
            cancelled = true;
    ...
    return (
        <>
            <div className="flex flex-col flex-1 min-h-0">
                ...
                <div className="shrink-0 px-6 pt-5 pb-3 flex flex-col gap-4">
                    <p className="flex items-start gap-2 text-sm text-muted-foreground leading-relaxed">
                        ...
                        />
                        <span>
                            The Patreon URL panel needs your Patreon session cookie. The Drive
                            download flow needs your Google cookies. The{" "}
                            <strong className="text-foreground font-medium">
                                browser extension
                            </strong>{" "}
                            syncs both with one click and is the recommended way; see "How do I sync
                            my cookies?" below.
                        </span>
                    </p>
                    ...
                    <div className="grid grid-cols-[minmax(0,5rem)_1fr_auto] gap-x-3 gap-y-2 items-center">
                        <StatusRow
                            ...
                            onClear={
                                patreonStatus?.set ? () => setClearPatreonOpen(true) : undefined
                            }
                            busy={saving}
                        />
                        <StatusRow
                            ...
                            onClear={googleStatus?.set ? () => setClearGoogleOpen(true) : undefined}
                            busy={saving}
                        />
                    </div>
                </div>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto px-6 pb-3 flex flex-col gap-4">
                    <div className="flex flex-col gap-2">
                        ...
                        <p className="text-xs text-muted-foreground/80">
                            Google cookies can't be pasted by hand (they're an array of structured
                            entries). Use the browser extension to sync both at once.
                        </p>
                    </div>

                    {feedback && (
                        <StatusLine tone={feedback.type === "success" ? "success" : "error"}>
                            {feedback.msg}
                        </StatusLine>
                    )}
                            ...
                            <div className="mt-2 bg-muted/40 border border-border rounded-md p-4 text-sm text-muted-foreground leading-relaxed flex flex-col gap-4">
                                <section className="flex flex-col gap-2">
                                    ...
                                    <p>
                                        ...
                                        syncs your Patreon and Google session cookies in one click.
                                        Both are needed for the app's two main workflows (Patreon
                                    ...
                                    <ol className="list-decimal pl-5 space-y-1.5">
                                        <li>
                                            ...
                                            on Firefox), enable Developer mode, and load the
                                            unpacked{" "}
                                            <code className="font-mono text-foreground/80">
                                                extension/
                                            </code>{" "}
                                        ...
                                        <li>
                                            Visit any Patreon page. Click the floating{" "}
                                            <strong className="text-foreground">
                                                Sync Patreon cookie
                                            </strong>{" "}
                                            pill (or open the extension popup and click{" "}
                                            <strong className="text-foreground">
                                        ...
                                        <li>
                                            Re-sync when fetches start failing (Patreon sessions
                                            last about a month, Google about two weeks).
                                        </li>
                                    </ol>
                                ...
                                <section className="flex flex-col gap-2 pt-3 border-t border-border">
                                    ...
                                    <p>
                                        Google cookies are an array of structured entries; they
                                        can't be pasted as plain text and must be synced via the
                                        extension. For Patreon only, copy the cookie string from
                                        DevTools:
                                    </p>
                                    <ol className="list-decimal pl-5 space-y-1.5">
                                        <li>
                                        ...
                                        <li>
                                            Filter to{" "}
                                            <strong className="text-foreground">Doc</strong> and
                                            reload the page so the document request appears.
                                        </li>
                                        <li>
                                            Click the document request,{" "}
                                            <strong className="text-foreground">Headers</strong>,
                                            scroll to{" "}
                                            <strong className="text-foreground">
                                                Request Headers
                                            </strong>
                                            ...
                                            line,{" "}
                                            <strong className="text-foreground">Copy value</strong>.
                                        </li>
                                        <li>
                                            Paste the entire value into the box above and click{" "}
                                            <strong className="text-foreground">
                                                Save Patreon cookie
                                            </strong>
            ...
            <AlertDialog open={clearPatreonOpen} onOpenChange={setClearPatreonOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Clear the saved Patreon cookie?</AlertDialogTitle>
                        <AlertDialogDescription>
                            The Patreon URL fetch flow will stop working until you sync a new
                            cookie.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <AlertDialog open={clearGoogleOpen} onOpenChange={setClearGoogleOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Clear the saved Google cookies?</AlertDialogTitle>
                        <AlertDialogDescription>
                            The Drive download flow will stop working until you sync new cookies via
...
interface StatusRowProps {
    label: string;
    status: "checking" | "not-connected" | { kind: "connected"; detail: string };
    onClear?: () => void;
    busy: boolean;
}
...
function StatusBody({ status }: { status: StatusRowProps["status"] }) {
    ...
    return (
        <span className="flex items-center gap-2 text-sm text-success">
            ...
            <span>
                Connected, <span className="font-mono tabular-nums">{status.detail}</span>
            </span>
        </span>
    );
}
