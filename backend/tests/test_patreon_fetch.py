...
class TestPostIdFromUrl:
    ...
    def test_creator_url_returns_none(self):
        ...
        assert _post_id_from_url("https://www.patreon.com/testartistasmr") is None
...
class TestFindCachedPost:
    ...
    def test_finds_sidecar_by_post_id(self, tmp_path: Path):
        ...
        _write_sidecar(post_dir, "12345", title="Cached Post", artist="Test Artist")
    ...
    def test_finds_audio_in_flattened_download_path(self, tmp_path: Path):
        ...
        flat_dir = tmp_path / "Test Artist" / "12345 - Cached Post"
    ...
    def test_finds_audio_in_legacy_flat_layout(self, tmp_path: Path):
        ...
        output_dir = tmp_path / ".patreon-dl"
        post_dir = output_dir / "Patreon" / "creator" / "posts" / "12345"
        _write_sidecar(post_dir, "12345", title="Old Post", artist="Test Artist")
...
class TestFlattenAudio:
    def test_uses_creator_postid_title_layout(self, tmp_path: Path):
        ...
        expected = tmp_path / "Test Artist" / "12345 - Cached Post" / "song.mp3"
    ...
    def test_drops_title_suffix_when_title_empty(self, tmp_path: Path):
        ...
        expected = tmp_path / "Test Artist" / "22222" / "song.mp3"
        assert expected.is_file()
...
class TestVanityFromUrl:
    def test_extracts_vanity_from_c_path(self):
        url = "https://www.patreon.com/c/testartistasmr/posts?vanity=testartistasmr"
        assert _vanity_from_url(url) == "testartistasmr"

    def test_extracts_vanity_from_legacy_short_path(self):
        assert _vanity_from_url("https://www.patreon.com/testartistasmr") == "testartistasmr"
...
def _write_creator_sidecar(
    ...
    *,
    ...
    sidecar = {
        ...
        "included": [
            {"type": "user", "id": "u1", "attributes": {"full_name": "Test Artist"}},
...
class TestFindCachedCreatorPosts:
    def test_returns_empty_when_nothing_cached(self, tmp_path: Path):
        assert _find_cached_creator_posts(tmp_path, "testartistasmr", None, None) == []
    ...
    def test_returns_all_posts_for_creator(self, tmp_path: Path):
        ...
        found = _find_cached_creator_posts(output_dir, "testartistasmr", None, None)
    ...
    def test_slugified_artist_fallback_when_campaign_missing(self, tmp_path: Path):
        ...
        # ("Test Artist ASMR" → "testartistasmr") and matches that against
        # the URL vanity.
        output_dir = tmp_path / ".patreon-dl"
        ...
        sidecar_no_campaign = {
            ...
            "included": [
                {"type": "user", "id": "u1", "attributes": {"full_name": "Test Artist ASMR"}},
            ],
        }
...
class TestParseProgressLine:
    ...
    def test_matches_targeting_creator(self):
        line = "Jan 20 14:32:11: info: PostDownloader: Targeting posts by 'testartistasmr'"
        assert parse_progress_line(line) == {
            "state": "resolving",
            "target_kind": "creator",
            "label": "testartistasmr",
        }
    ...
    def test_matches_done_downloading(self):
        line = "Jan 20 14:32:35: info: PostDownloader: Done downloading posts by 'testartistasmr'"
