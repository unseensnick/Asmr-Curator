...
- **Move-to-library flow inside the work area.** Every selected file carries a `Move to library` collapsible with a folder-tree picker rooted at `LIBRARY_PATH`. Breadcrumb navigation, drill-in by click, inline `+ New folder` (creates under the currently-viewed subdir and auto-drills into it), and an optional `↗ Open in OS` button that opens the host file manager at the current folder (hidden in Docker / devcontainer where the host's file manager isn't reachable). The `Move here` button commits — and if the user has a generated filename preview that differs from the current name, an inline checkbox offers to apply that rename during the move so it lands as a single server call.
- **Standalone `+ New folder` button on the Library tab.** Inline input beside the search row creates a subfolder at the library root without needing to start a move flow. Same `/api/mkdir` endpoint the picker reuses.
- **Patreon Apply bridge link.** After Apply, a small `↗ Rename and move <filename>` link appears under the apply-status banner. Clicking it jumps to the FileBrowser Downloads tab, selects the downloaded file, and scrolls into view so the rename + move flow is one click away from finishing the workflow.

### Changes

- **API surface gained a `root` parameter on every file endpoint.** `/api/files`, `/api/files/search`, `/api/files/debug`, `/api/rename`, and `/api/convert` accept `root: "library" | "downloads"` (defaults to `"library"`). New endpoints: `POST /api/mkdir`, `POST /api/move`, `POST /api/system/explore`, plus `os_explorer` capability flag on `GET /api/system/info`. The existing client contract for the default-`library` use cases is unchanged; new callers (Downloads tab, move picker, Patreon ingest endpoints) supply the root explicitly.
