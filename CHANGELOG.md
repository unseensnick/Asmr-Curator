...
- **Right-click → "Add as alias of…" on an unrecognised tag chip now actually opens the picker.** Previously the context menu flashed the next step then immediately dismissed — the trailing pointer events from the menu-item click were closing the just-opened popover as "outside interaction." The picker is now deferred past the click cycle and guarded against the close-on-trailing-event race.
