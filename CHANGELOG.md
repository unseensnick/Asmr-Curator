...
- **Opening the Dictionary from inside Bulk edit is smooth now.** Previously the Bulk edit sheet blinked out for a frame and the Dictionary flashed in before its slide-in animation ran. The two sheets share a code chunk loader, and one Suspense boundary was suspending both. Each lazy sheet now has its own boundary; the Dictionary chunk also pre-warms on hover / focus and on Bulk edit open, so the slide-in starts on click rather than after a chunk-load gap.
- **The Dictionary itself opens faster on first click.** The three panes (Vocabulary, Suppressed, Test extraction) used to render up-front so tab switches felt instant, but that meant a multi-hundred-entry dictionary did all that work before the slide-in animation could start. Inactive panes now mount on first visit instead — only the Vocabulary pane renders when the sheet opens; the others mount when you click their tab. Once a tab has been visited, switching back is still instant because the pane stays mounted.
...
- **Drag-selecting bottom-to-top in the FileBrowser no longer drops the first row.** Mounting the multi-selection action bar above the list shifted the rows down by its height, and the rubber-band rect (anchored to the original mousedown viewport position) lost track of the row beneath the cursor. The drag origin now tracks the container so the originally clicked row stays inside the rect across the layout shift.
- **Drag-select's auto-scroll stops at the bottom of the list.** Hovering at the bottom edge after scrolling to the end could spin the auto-scroll forever — the translucent rectangle was rendered inside the scroll container and its growth kept extending `scrollHeight`, so the "no more room to scroll" check never tripped. The overlay is now viewport-anchored and the auto-scroll tick bails as soon as the container hits its scroll bounds.
- **Click jitter no longer toggles a row into multi-select mode.** A small hand-tremor on what was meant to be a normal click could cross the rubber-band's engage threshold and add the clicked row to a batch selection; the threshold is now higher so deliberate drags still engage but accidental jitter doesn't.
