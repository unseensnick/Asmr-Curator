...
- **All tooltips inside the Bulk edit sheet now use the same hover affordance.** Header buttons, the load / clear actions, and the per-row remove button switched from the browser's native `title=""` (inconsistent timing, no styling) to the same delayed Radix tooltip the per-row filename already used, so hover behaviour stays homogenous throughout the sheet.
