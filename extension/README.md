...
4. The extension captures the playback URL the moment its size reaches
   ...
   it to the backend. The file lands at `DOWNLOAD_PATH/<post_id>/<filename>`
...
- `POST /api/patreon/ingest-external-audio` — `{ post_id, source_url }`,
  downloads the URL into `DOWNLOAD_PATH/<post_id>/`.
- `GET  /api/files?root=downloads` — used to list existing `post_id`
  directories under `DOWNLOAD_PATH` when no parent post was detected.
